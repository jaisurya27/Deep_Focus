"""Shared helpers for OpenAI-compatible chat completions (streaming + non-streaming)."""
...
async def fetch_openai_compatible_chat(
    *,
    base_url: str,
    api_key: str,
    model: str,
    ...
    """Non-streaming POST — yields the full response as a single chunk.

    Used for providers (e.g. xAI/Grok) that don't support SSE streaming.
    """
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if response.status_code >= 400:
            raise RuntimeError(
                f"Upstream {response.status_code} from {url}: {response.text[:500]}"
            )
        data = response.json()
